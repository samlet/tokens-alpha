...
contract OracleResolverI {
    function getOracleAddress() constant returns(address);
}

// 給dapp開發者使用的合約
contract UsingMyOracle {
    OracleResolverI resolver;
    OracleI oracle;

    modifier myOracleAPI {
        if (address(resolver) == 0) {
            // 指定OracleResolver的合約地址，要替換成你自己的
            resolver = OracleResolverI(0xf915602Df295BdcB1cDB50cd0d1C8Ab3DcB8e271);
            oracle = OracleI(resolver.getOracleAddress());
        }
        _;
    }
